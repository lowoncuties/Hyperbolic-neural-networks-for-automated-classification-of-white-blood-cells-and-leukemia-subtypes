...
def _env_bool(name: str, default: bool) -> bool:
    ...
    if value is None:
        return default
    return value.strip().lower() not in _ENV_FALSE


def default_num_workers() -> int:
    override = os.environ.get("WBC_DATALOADER_WORKERS")
    ...
    return min(24, cpu_count // 4)


DEFAULT_DATALOADER_WORKERS = 32
DEFAULT_DATALOADER_PREFETCH = max(2, _env_int("WBC_DATALOADER_PREFETCH_FACTOR", 4))
DEFAULT_DATALOADER_PERSISTENT = _env_bool("WBC_DATALOADER_PERSISTENT_WORKERS", True)
...
def loader_kwargs(
    ...
    if workers > 0:
        kwargs["persistent_workers"] = DEFAULT_DATALOADER_PERSISTENT
        kwargs["prefetch_factor"] = DEFAULT_DATALOADER_PREFETCH
    return kwargs
...
def build_dataloaders_from_splits(
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        **loader_kwargs(workers, pin_mem, worker_init, generator=gen),
    )
    val_loader = DataLoader(
        ds_val,
        batch_size=batch_size,
        shuffle=False,
        **loader_kwargs(workers, pin_mem, worker_init),
    )
    ...
    print(
        "  loader : "
        f"workers={workers}, persistent={DEFAULT_DATALOADER_PERSISTENT if workers > 0 else False}, "
        f"prefetch={DEFAULT_DATALOADER_PREFETCH if workers > 0 else 0}, pin_memory={pin_mem}"
    )
...
__all__ = [
    ...
    "DEFAULT_DATALOADER_PREFETCH",
    "DEFAULT_DATALOADER_PERSISTENT",
