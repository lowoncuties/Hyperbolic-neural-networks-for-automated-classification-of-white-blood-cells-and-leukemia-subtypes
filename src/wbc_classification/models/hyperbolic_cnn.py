...
class HyperbolicPrototypeHead(nn.Module):
    ...
    def __init__(
        ...
        self._eps_tau = 1e-6
        self.feature_clip_norm = (
            None if feature_clip_norm is None else float(feature_clip_norm)
        )
    ...
    def maybe_clip_features(self, euclidean_feats: torch.Tensor) -> torch.Tensor:
        """Clip Euclidean feature norms before mapping them into the manifold."""

        if self.feature_clip_norm is None or self.feature_clip_norm <= 0:
            return euclidean_feats
        norms = euclidean_feats.norm(dim=-1, keepdim=True).clamp_min(1e-12)
        max_norm = torch.full_like(norms, self.feature_clip_norm)
        scale = torch.minimum(torch.ones_like(norms), max_norm / norms)
        return euclidean_feats * scale
    ...
    def forward(self, euclidean_feats: torch.Tensor) -> torch.Tensor:  # type: ignore[override]
        ...
        euclidean_feats = self.maybe_clip_features(euclidean_feats)
    ...
    def __init__(
        ...
        **head_kwargs,
        ...
        self.head = HyperbolicPrototypeHead(
            ...
            feature_clip_norm=feature_clip_norm,
            **head_kwargs,
        )
