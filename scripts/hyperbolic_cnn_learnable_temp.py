...
Extension: learnable temperature (global scalar) inside the hyperbolic head.

- Temperature is parameterized as tau = softplus(raw_tau) + eps  (always > 0)
- Included in optimizer param groups with its own LR (lr_temperature)
- Logged to metrics.csv and stored in checkpoints
"""
...
class HyperbolicPrototypeHead(nn.Module):
    """
    Hyperbolic prototype head with:
      - learnable curvature (raw_c -> softplus -> clamp)
      - learnable temperature (raw_tau -> softplus)
    """
    def __init__(
        self,
        embedding_dim: int,
        ...
        # Curvature parameterization: c > 0
        self.raw_c = nn.Parameter(torch.log(torch.tensor(float(init_curvature))))

        # Temperature parameterization: tau > 0
        # Use inverse softplus for more faithful init, fallback if init_temperature is tiny
        init_temperature = float(max(init_temperature, 1e-6))
        inv_softplus = lambda x: torch.log(torch.expm1(torch.tensor(x)))
        try:
            raw_tau0 = inv_softplus(init_temperature)
        except Exception:
            raw_tau0 = torch.tensor(np.log(np.exp(init_temperature) - 1.0), dtype=torch.float32)
        self.raw_tau = nn.Parameter(raw_tau0.to(torch.float32))

        # Prototypes in tangent space at origin
    ...
    def __init__(
        ...
        **head_kwargs,
        ...
        self.head = HyperbolicPrototypeHead(
            ...
            init_temperature=temperature,  # <-- initial temperature
            **head_kwargs,
        )
...
class FinetuneConfig:
    ...
    lr_temperature: float = 1e-3  # <-- NEW: LR for learnable temperature
    img_size: int = 224
    workers: int = 4
    seed: int = 42
...
def train(
    ...
    model = HyperbolicClassifier(
        ...
        temperature=cfg.temperature,  # initial temperature
    ).to(device)
    ...
    tau_param  = [model.head.raw_tau]  # <-- NEW
    ...
    optimizer = torch.optim.Adam(
        [
            ...
            {"params": tau_param,           "lr": cfg.lr_temperature},  # <-- NEW
        ]
    )
    ...
    if cfg.out_dir:
        ...
        if not os.path.exists(metrics_csv):
            ...
            with open(metrics_csv, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "epoch","split","train_loss","accuracy","f1_macro",
                    "curvature_c","temperature_tau",
...
def run_sweep_and_save_csv(
    ...
    summary_fields = [
        ...
        "best_epoch", "best_val_acc", "test_acc", "test_f1_macro", "learned_c", "learned_tau",
    ...
    for params in iter_grid(grid):
        ...
        cfg = FinetuneConfig(
            ...
            temperature=params["temperature"],  # initial temp
            batch_size=params["batch_size"],
            epochs=run_epochs,
            lr_backbone=params["lr_backbone"],
            lr_head=params["lr_head"],
            lr_curvature=params["lr_curvature"],
            lr_temperature=params.get("lr_temperature", 1e-3),  # NEW (optional in grid)
...
if __name__ == "__main__":
    param_grid = {
        ...
        # refine around the winners
        "lr_backbone":    [2e-4, 1e-4], #3e-4
        "lr_curvature":   [8e-3, 4e-3, 5e-3, 1e-2], #6e-3, 
        "lr_head":        [2e-3, 1e-3, 3e-3, 4e-3, 5e-3], #1e-2

        # NEW: try a couple of tau learning rates (optional but recommended)
        "lr_temperature": [1e-4, 3e-3, 3e-3],
    }
